...
|---|---|---|---|
| Headline | `lib.headline(pageView, {attention, question, busy, tool, finishedAt, now})`: talking > approval ("Approve in the terminal"; question: "Answer in the terminal") > **Muted** > "Claude is <claudeVerb(tool)>" > "Claude finished" (30 s) > "Listening"; a card view shows its header label. Muted before Claude's news is a deliberate change to IMPLEMENTATION §6 (the user must not talk into a muted mic unaware) | Done | Port `headline` |
| Header word | `lib.statusWord`: "Needs you" while an approval waits (live), else the header label | Done | Port |
| Caption line | One line under the string: `lib.captionNote` (muted: "Sotto can't hear you. Still billing. Press M to listen."; connecting: the sub) else the latest caption; the voice labelled with the persona's name | Done | Port |
| Claude row | `lib.claudeHead`: moon phase new / waxing / eclipse / full, "Claude · Working", "Claude is waiting for you"; timer m:ss (`formatClock`), counts the wait in approval | Done (SVG moon, 12 px) | `MoonGlyph` |
| Eclipse timing | The string starts at the event; words, header, Claude row and the question switch at totality (750 ms, `lib.ECLIPSE_TOTALITY_MS`); state already present at launch and Reduce Motion: at once | Done (`data-eclipsing`, `data-eclipse`) | Same timing |
| Milestone stars | §5.4 rule above | Done (memory only) | Port in `StateModel` |
| Cost | Idle 0 fps; working bead 10 fps; approval shimmer only (web: 24 fps clipped to the corona box); sleeping ≤ 15 fps | Done, `test/web/string.test.js` | `TimelineView(paused:)` |
| Tokens | IMPLEMENTATION §2 (ground `#05070C` / `#FBFCFD`, need `#FFB547` / `#C4800E` + text `#A55200`) | Done (`styles.css`) | `Theme` |
| Footer | Persona chip (tuning glyph, name, voice) opens Settings; Pause, End voice, gear as 40 pt icon buttons; the gear left the header | Done | Same |
