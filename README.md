...
- **AirPods and other Bluetooth headsets:** macOS's voice processing, which the app's web view uses for the microphone, switches a Bluetooth headset that is the *default input* into its lower-quality headset mode for the whole session, and it lowers other apps' audio (by about 15 dB) while the mic is open, which with voice wake means for as long as voice is on. So with `window` = `auto` (the default), the plugin uses Chrome when your default input is Bluetooth. To use the app with AirPods anyway, set `window` to `app`; making the MacBook microphone the input in System Settings → Sound should avoid the headset mode (expected from the measurements, not yet verified).
