...
- **The desktop app** (WKWebView) routes capture through Apple's voice-processing unit, so `echoCancellation` there is Apple's canceller (**measured**). Its cost: a Bluetooth headset that is the default input switches to its lower-quality headset profile, and other apps' audio is lowered while the mic is open, and WebKit offers no way to steer or tune that unit.
- So on **headphones** the app captures the microphone itself (the built-in mic when the default input is Bluetooth) with a plain input unit, no voice processing, and streams it into the page as an ordinary track. Headphones have no echo path, so nothing is lost; the headset keeps its high-quality profile and nothing is ducked. On speakers the app keeps WebKit's capture and Apple's canceller. `auto` therefore picks the app with AirPods too, and falls back to Chrome only when there is no other input or the output is not headphones.
- A simulated test (the model's own voice mixed back into the native mic, a pure 40 ms delayed copy, at -10 dB and at 0 dB) did not make gpt-live-1 transcribe or interrupt itself. That is encouraging but not proof: real speakers add reverb and distortion, which is exactly what echo cancellers exist for. Speakers keep AEC.
