...
| `set_cap` | `minutes` (0 = unlimited) | `voice.setCap(minutes, "app")`; error `bad_cap` | `{minutes, message}` (added in 0.4.7) |
...
- Views follow the web page's information design, natively:
  ...
  - the string (`FilamentStage`: one `Canvas` in `TimelineView(.animation(minimumInterval:paused:))`, paused when nothing moves; 12 fps breathing while live and listening (v0.4.1); 10 fps while only Claude's bead travels; 15 fps sleeping shiver; 60 fps speech and one-shots) driven by the mic, speaker and wake levels (raw RMS from SottoAudio, converted with `ViewText.levelFromRms` and gated like the page), with the peg as the mute control
...
| Claude card height (user reports on 0.3.1 and 0.4.1) | One fixed region that scrolls (`panel.pageFollow`), a thin auto-hiding thumb, "Jump to latest" | Grew with long output inside the panel's scroll view; a scroller sat over the text; then a clamp with More | The same fixed region between the Claude row and the footer, an `NSScrollView` with the overlay scroller pinned (hidden at rest) in a 12 pt gutter, edge fades, follow mode and "Jump to latest" (`ClaudeCardLayoutTests`: a long turn scrolls in a stable frame, follow and unfollow, jump, the approval at 360 x 420; snapshots 25/26) |
...
| Cost | Idle 0 fps; working bead 10 fps; approval shimmer only (web: 24 fps clipped to the corona box); sleeping ≤ 15 fps | Done, `test/web/string.test.js` | Done: `FilamentEngine.rate` (0 fps idle except the 12 fps listening breath, 10 fps bead, ≤ 15 fps sleep, corona quad at 30 fps), paused when hidden or occluded |
| Tokens | IMPLEMENTATION §2 (ground `#05070C` / `#FBFCFD`; native light is `#FFFFFF` since v0.4.1, need `#FFB547` / `#C4800E` + text `#A55200`) | Done (`styles.css`) | Done: `HybridTheme` (the old `Theme` stays for Settings) |
...
- **Views:** `PanelView` places every zone in a fixed box (`HybridLayout`); `FilamentStage` is one `Canvas` in `TimelineView(.animation(minimumInterval:paused:))` over the whole panel (string, tides, persona ghost, stars, bead, frame, peg, eclipse), plus `CoronaLayer`, a 110 pt quad around the peg that alone animates (30 fps) while an approval holds. `ClaudeColumn` holds the row and the page (`ClaudePage`: Claude's messages for this turn and the recent ones in New York, in a fixed region that scrolls (`ClaudeScroll`, an `NSScrollView` with the overlay scroller) and follows the newest words, with "Jump to latest" when the reader scrolls up; SPEC-DEVIATIONS "Claude's page scrolls"; `ApprovalBody`: the command in bare mono, the cwd, Claude's reason, "Show terminal"; `StateBody`: a card's text on the page). `MiniPanelView` is the compact strip (replaces the 290 x 44 pill). `FooterDevices.swift` holds the footer's microphone and speaker buttons and their quick picker (`DeviceMenu`: System default first, the check, the fallback when a chosen device goes away; SPEC-DEVIATIONS "Devices in the footer").
