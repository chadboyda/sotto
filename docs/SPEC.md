...
A persona is the voice's identity and personality in the Live instructions (§8.1) plus an optional suggested voice. It changes how the voice talks (tone, humor, opinions, emotion, pace, pitch, pauses, voiced sounds such as laughs, sighs, hums and whispers), never what it relays: the identity line names the voice after the persona ("You are Moss, the voice of Claude Code"), the block follows as its "Personality and Tone" with a fixed "Staying in character" frame, and every rule that follows (brevity, secrets, mic checks, relay and "done" rules, delegation policy) decides what is said and is stated to take precedence on content, never to ask for a neutral voice. Measured live on 2026-09-25 (ten built-ins, the same greeting, handoff and one-failure relay each, judged blind): with ~250-token bodies framed as a footnote, a judge matched transcripts to personas 43% of the time; with the bodies and frame below, 93-100%, and an audio judge rated the new delivery stronger for 10 of 10.
- **Built-ins** (id, suggested voice): `sotto` (marin, default), `june` (coral), `moss` (cinder), `tempo` (quartz), `koan` (stone), `vic` (meridian), `pip` (verse), `fern` (ballad), `lark` (gleam), `vela` (willow); voices may be shared. Each is chosen for fit: presentation from the OpenAI voice table first (Lark and Vela read clearly feminine: measured median pitch over 180 Hz), then pitch, then texture from a blind 3-run audio ranking of the voice samples. Each has a display name, a one-line description and a body of at most ~750 tokens: a one-line `Voice:` direction, who it is, delivery (pace, pitch and energy, pauses, voiced sounds, written as directions, never bracketed tags, which would be read aloud), signature habits, and sample lines for every kind of moment (greeting, handing work to Claude "said as you delegate it", good result, failure, approval with "approval" and "terminal", quick acks). Sample details are made up and differ from real results. Sounds are occasional and never over a failure or a stressed user. toggle.sh mirrors the ids, voices and descriptions (pinned by test).
- **Custom:** `<project>/.claude/sotto-personas/<id>.md` > `D/personas/<id>.md` > built-in, by id; the file name is the id (`^[a-z0-9][a-z0-9_-]{0,31}$`). Optional `---` frontmatter with `name`, `description` (≤ 160 chars) and `voice` (one of the 22; anything else is ignored with a `persona.warning`), then the body. An empty body is skipped (`persona.invalid`); a body over 4,000 chars is cut at a word; `{{`/`}}` are removed. At most 50 files per directory. Custom personas follow the built-ins, sorted by id. The project is the owner's `project_dir` (else `cwd`); unbound, the `/control` caller's session.
...
- **Voice wake:** the page detects speech and POSTs `/api/session` with `reason:"wake"`.
  ...
  - No clip within 12 s of ready, an invalid clip, a failed or empty transcription: the instruction says the user's first words were not captured; if they are still talking, listen and respond; if they are quiet, say only a short "Yes?" so they know the voice is listening (never silence: the user spoke to wake it).
...
- **False wakes:** a session woken by voice in which no user words arrive (no input transcript delta with a letter or digit, no clip transcript) and whose local mic detector (the page's `activity`, the app's relayed mic, §6.19) has heard no speech for `FALSE_WAKE_QUIET_MS` (10 s), counted from the later of the wake, the last local speech and the model's last words (so the user can answer anything it just said), sleeps as soon as it has been live 15 s (`false_wake`); while the local mic still hears speech the normal idle rule applies. An empty clip transcript alone never makes a wake false. The `WakeGovernor` then raises the page's SNR bar by 3 dB (max 6) and sets a cooldown before the next voice wake: 5 s, 15 s, then 30 s for consecutive false wakes. A wake that hears words resets the count and lowers the boost by 1.5 dB.
...
- **Default mic** (`lib.pickInputDevice(devices, savedId)` → `{deviceId, rule, hint, label, savedMissing}`):
  ...
  2. else the system default input (the `"default"` pseudo device of Chrome and the app, label `Default - <name>`), whatever it is: a Bluetooth headset's mic is used when it is the default (no automatic override);
  3. no pseudo default (other browsers): the first input listed (the browser's default).
  ...
  When a remembered mic is missing, show "The microphone you chose isn't connected. Using <name>." The drawer names the mic in use ("In use: <name>."), the mute button's tooltip too, and the select shows "Automatic (<name>)" and "System default (<name>)". On `devicechange` while live, the rules run again and the mic switches (`replaceTrack`) when they now resolve to a different device (the macOS default changed, a remembered mic came back, the one in use vanished); while sleeping, the wake listener's held mic is released and listening restarts on the new choice (log `mic: default_changed while sleeping`).
...
### 6.19 Uplink level and per-device mic calibration (`daemon/agc.js`, native app only)
gpt-live-1 does not hear quiet speech: on the native path, speech fixtures as a Bluetooth headset delivers them were transcribed with 100 % word recall at -31 and -40 dBFS, 97 % at -45 and 0 % at -50 (no transcript, no reply; `test/e2e/native-bt.mjs`). The app captures raw (no AGC); Chrome's AGC hides this on the page. AirPods Max in call mode deliver conversational speech around -30 dBFS (loudest frames -15 to -21; a `sotto debug capture` of the user, 2026-09-25), well above that floor. The "-46 to -51 dBFS speech" 0.4.3 learned there was keyboard typing and breath: pitch periodicity about 0.38 and 13 % of the power under 1 kHz, against 0.75 and 95 % for the voice. So only voiced speech teaches the level.

- **AGC** (`createUplinkAgc`, per 20 ms mic frame of the app, before the pacer): levels are measured on a 150 Hz high-passed copy (the wake detector's units); a frame is active when it is ≥ 12 dB over the floor (the floor counts as no lower than -85; digital zeros count as silence) and ≥ -68 dBFS, and voiced when it is active with pitch periodicity ≥ 0.5 and ≥ 50 % of its power in 80-1000 Hz (`frameFeatures().voiceRatio` over the frame plus the previous one's tail, 512 samples). A run of active frames (gaps ≤ 200 ms, ≥ 400 ms) is an utterance when ≥ 30 % of its frames are voiced (speech: 60-70 %; typing: about 5 %), else it is logged as `mic.not_speech {input, ms, db, voiced, count}` (the first three, then every twentieth) and teaches nothing. The speech level is the median of the voiced frames of the last learned utterances (250 frames); an unknown device gets a first estimate inside its first utterance once it has 25 voiced frames making ≥ 30 % of it. The gain brings it to -26 dBFS: never negative, at most 30 dB, never lifting the floor above -52 dBFS, not applied under 3 dB (a normal mic passes byte for byte), slewing 15 dB/s up and 75 dB/s down (the first estimate sets it at once), ramped within each frame and limited to a -1 dBFS peak. Only the uplink is changed; the wake detector, can't-hear monitor and echo report see the raw mic. Learning pauses while the assistant is audible and until the stream has settled.
- **Settle** after a `route` (the app rebuilt its capture): the wake detector, its floor and calibration ignore the stream until it has carried non-zero audio for 200 ms (800 ms for a Bluetooth input; a profile switch delivers nothing or zeros for up to about 2.6 s), or 5 s passed. A detector armed on the old stream is re-armed on the settled one (fresh warm-up and pre-roll); a clip being captured keeps recording. Log `native.settled {input, bluetooth, ms, audio}`.
- **Per-device calibration** (`createMicProfiles`, `D/mic-levels.json` `{devices: {<input name>: {speech_db, floor_db, utterances, updated}}}`, at most 20 devices): each utterance (above) heard while live updates the device's speech level (running mean over the last 12, from its voiced frames). Profiles carry `method: "voiced"`; one without it (0.4.3, learned from typing) is ignored and relearned. Five utterances make it calibrated; the AGC then starts at the right gain on that device, and the wake detector's level bar becomes `speech_db` minus 10/15/20 dB (low/medium/high) instead of the preset `minDb` (§7.6). Log `mic.calibrate {input, utterance_db, utterance_ms, voiced, speech_db, floor_db, utterances, calibrated, gain_db}` for the first five utterances, then every tenth.
- **Talking** is a voiced frame with ≥ 8 of the last 25 active frames voiced (typing: 0-2). **Local speech** while live (talking, not while the assistant is audible) is reported to `voice.noteLocalSpeech()` at most every 250 ms: it holds off idle sleep and the false-wake verdict (§6.15) like the page's `activity`. The can't-hear monitor (§7.5) counts the app's mic as the user talking only within 300 ms of talking (`sample({speech})`): typing at the mic is not "6 s of speech with no transcript".
- `/status` `native.agc {gain_db, speech_db, floor_db}`; the `native.audio` log line carries the same.
- **Debug capture** (`daemon/debugcapture.js`, opt-in, off by default): `sotto debug capture on` creates `D/debug/capture.on`; while it exists the native session writes `D/debug/<stamp>-raw.wav` (the app's mic frames as received, zeros while muted) and `<stamp>-up.wav` (what was pushed to gpt-live-1, silence while no session was live), both 24 kHz mono PCM16 and sample-aligned. The daemon looks at the flag once a second, deletes it 10 minutes after the capture started (a flag over an hour old when first seen starts nothing), and deletes captures older than 24 h; `sotto debug capture off` stops and deletes them. Log `debug.capture {on}`, `debug.capture_saved {raw, up, seconds}`. Nothing leaves the machine.
...
- **VAD** (`createVad`, pure, time in samples): per frame, level (dBFS), share of power in 80-4000 Hz, spectral flatness there, and pitch periodicity (normalized autocorrelation, 70-400 Hz). A frame is voiced when level ≥ `minDb`, level − noise floor ≥ `snrDb` + `boost_db`, band share ≥ 0.5, periodicity ≥ 0.35, flatness ≤ 0.45. The noise floor follows unvoiced frames (fast down, ~2.7 s up). A speech run tolerates 160 ms gaps; it wakes once when voiced time ≥ `minSpeechMs`, ≥ 55 % of its frames are voiced and its level swings ≥ 5 dB (syllables; steady tones don't). Short runs up to 700 ms before it extend the onset ("Hey, … can you"). Presets: low `{16 dB, −46 dBFS, 420 ms}`, medium `{11, −54, 320}`, high `{7, −62, 240}`. The swing leaves out a run's first two frames (a steady tone that starts abruptly is not a syllable). Digital zeros never set or move the floor. **Native profile** (`profile: "native"`, the app's raw mic relayed to the daemon, docs/NATIVE.md §4.5): the detector analyses a 150 Hz high-passed copy (4th-order Butterworth; analysis only: the uplink gets only the §6.19 gain) with presets low `{11 dB, −54, 400}`, medium `{7, −62, 300}`, high `{5, −68, 240}`, a voiced frame must also keep ≥ 40 % of its power in 80-1000 Hz (reason `voice_band`: keyboard typing at a headset has pitch-like periodicity but little power there, and woke 0.4.3 with nothing to transcribe), and its floor comes from a 400 ms warm-up after arming (20th percentile, never the first frame, capped at the device's floor measured while live + 6 dB; that live floor is the 10th percentile over 8 s). A calibrated device (§6.19) replaces `minDb` with its learned speech level minus 10/15/20 dB (low/medium/high); the other gates are unchanged. A candidate that came within 5 dB of the bars for ≥ 120 ms without waking is logged as `wake.near {ms, voiced_ms, level_db, snr_db, floor_db, bar_db, min_db, reason}`: the first five per sleep at info level, the rest at debug. `wake.listen` carries `calibrated`, `speech_db`, `min_db`, `utterances`. Keyboard clicks (short, broadband, aperiodic), fans (floor) and steady music are rejected; TV speech is speech and may wake (false-wake back-off handles it).
...
`render({project, policyText, vocabulary, persona})` substitutes `{{name}}` (the persona's sanitised display name, `Sotto` without a persona), `{{project}}`, `{{policy_text}}`, `{{vocabulary}}` (the glossary, or nothing) and `{{persona}}` (`personaBlock(persona)`, §4.6: `Personality and Tone: you are <Name>.`, the body, then a fixed "Staying in character (<Name>)" list: be <Name> in every utterance, including acks, mic checks, handoffs, relays, failures, approvals and fixed lines; the character is mostly delivery, played bigger than a neutral assistant; relayed results are where character matters most; laughs, sighs, hums, gasps and whispers are voiced, never spoken as words or read from brackets, and never over a failure or a stressed user; sample lines are a spirit, not a script, their details are made up, never reuse one word for word; saying you'll hand something to Claude is never the handoff itself, delegate first; an opinion is not the user's decision; the rules below decide what is said and take precedence on content, none asks for a neutral voice. Nothing without a persona). Measured live (2026-09-25): a model that was told to add "a few words of personality, then the substance" under a long generic rule set ("like a sharp colleague", sample reactions "oh nice") spoke in near identical words in every persona; the sample handoff lines alone, before the "delegate first" line, cut real delegations from 19/20 to 25/30. The rules open with `Rules for what you say.` and describe content only. Persona and vocabulary text are inserted by position, never scanned for placeholders. The headings `Backchannel policy:`, `Interruption policy:`, `Delegation policy:`, `Backend tools:`, `Delegate to the backend when:` and `Do not delegate to the backend when:` are **verbatim** from guide-live-prompting.md. The template ends with the guide's two closing lines. Full text:

```text
You are {{name}}, the voice of Claude Code, a coding agent working in the user's terminal on the project "{{project}}". The user is a developer talking with you hands-free while Claude Code does the work. You handle the spoken conversation; Claude Code reads code, runs commands, and makes changes.
...
| `start` (other policies) | `Greet the user in one short sentence, fully in character, and mention that you're connected to Claude Code in {{project}}. Then stop and listen.` |
...
| `reconnect` | none |
| `reconnect` after a persona switch (§4.6; also when it changed the voice) | `In one short sentence, fully in your new personality and delivery, tell the user you're now <Name>. Then stop and listen; the conversation continues from where it left off.` |
...
- **Owner switch** (sent as a `session.commentary.append` through the speech queue, not as instructions; the new project's glossary follows as `session.thinking.append`): `The user switched to a different Claude Code session, in the project {{project}}. From now on, requests go to that session, and results for the previous project will not arrive. Briefly tell the user you're now connected to {{project}}.`
