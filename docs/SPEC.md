...
- **Built-ins** (id, suggested voice): `sotto` (marin, default), `june` (coral), `moss` (cedar), `tempo` (tempo), `koan` (sage), `vic` (ash), `pip` (echo), `fern` (verse), `lark` (shimmer), `vela` (vesper). Each has a display name, a one-line description and a body of at most ~250 tokens. toggle.sh mirrors the ids, voices and descriptions (pinned by test).
