...
final class StateModelTests: XCTestCase {
    ...
    func testActivityDrivesClaudeCard() {
        ...
        XCTAssertEqual(m.claudeCard.step, "Running the suite now.", "Claude's words, not the tool label")
        clock.t.addTimeInterval(21)
        XCTAssertEqual(m.claudeCard.step, "Running the suite now.", "a tool label never replaces Claude's words")
        XCTAssertEqual(m.claudeCard.secondary, false)
