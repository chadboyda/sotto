...
gpt-live-1 does not hear quiet speech: on the native path, speech fixtures as a Bluetooth headset delivers them were transcribed with 100 % word recall at -31 and -40 dBFS, 97 % at -45 and 0 % at -50 (no transcript, no reply; `test/e2e/native-bt.mjs`). The app captures raw (no AGC), and AirPods in call mode deliver conversational speech around -31 dBFS and soft speech at -46 to -51. Chrome's AGC hides this on the page.

- **AGC** (`createUplinkAgc`, per 20 ms mic frame of the app, before the pacer): levels are measured on a 150 Hz high-passed copy (the wake detector's units); a frame is speech when it is ≥ 12 dB over the floor (the floor counts as no lower than -85; digital zeros count as silence) and ≥ -68 dBFS. The speech level is the median of the last 250 speech frames (after 25). The gain brings it to -26 dBFS: never negative, at most 30 dB, never lifting the floor above -52 dBFS, not applied under 3 dB (a normal mic passes byte for byte), slewing 15 dB/s up and 75 dB/s down (the first estimate sets it at once), ramped within each frame and limited to a -1 dBFS peak. Only the uplink is changed; the wake detector, can't-hear monitor and echo report see the raw mic. Learning pauses while the assistant is audible and until the stream has settled.
...
- **Per-device calibration** (`createMicProfiles`, `D/mic-levels.json` `{devices: {<input name>: {speech_db, floor_db, utterances, updated}}}`, at most 20 devices): each utterance (≥ 400 ms of speech, gaps ≤ 200 ms) heard while live updates the device's speech level (running mean over the last 12). Five utterances make it calibrated; the AGC then starts at the right gain on that device, and the wake detector's level bar becomes `speech_db` minus 10/15/20 dB (low/medium/high) instead of the preset `minDb` (§7.6). Log `mic.calibrate {input, utterance_db, speech_db, floor_db, utterances, calibrated, gain_db}` for the first five utterances, then every tenth.
- **Local speech** while live (the AGC's speech frames, not while the assistant is audible) is reported to `voice.noteLocalSpeech()` at most every 250 ms: it holds off idle sleep and the false-wake verdict (§6.15) like the page's `activity`.
- `/status` `native.agc {gain_db, speech_db, floor_db}`; the `native.audio` log line carries the same.
- **Debug capture** (`daemon/debugcapture.js`, opt-in, off by default): `sotto debug capture on` creates `D/debug/capture.on`; while it exists the native session writes `D/debug/<stamp>-raw.wav` (the app's mic frames as received, zeros while muted) and `<stamp>-up.wav` (what was pushed to gpt-live-1, silence while no session was live), both 24 kHz mono PCM16 and sample-aligned. The daemon looks at the flag once a second, deletes it 10 minutes after it was made, and deletes captures older than 24 h; `sotto debug capture off` stops and deletes them. Log `debug.capture {on}`, `debug.capture_saved {raw, up, seconds}`. Nothing leaves the machine.
