...
- **Keychain in tests:** unit tests use `test/helpers/fake-keychain.js` (the daemon harness default) or a fake `security` via `SOTTO_SECURITY_BIN`; anything that spawns a real daemon sets `SOTTO_KEYCHAIN_SERVICE` to a throwaway name so the user's real `sotto` item never leaks in. Never point a Keychain *write* at a fake `HOME`: `security` then fails with "authorization was canceled by the user".
  ...
  - It also runs `test/e2e/native-bt.mjs` (a Bluetooth headset mic on the native path: speech fixtures as AirPods deliver them in call mode (8 kHz band, gated -80 dBFS floor) at -31 and -50 dBFS, scored by gpt-live-1's word recall (the -50 condition needs the uplink AGC, SPEC §6.19), then a voice wake at the headset level whose clip must be transcribed and whose session must stay awake while the user talks; ~110 billed seconds); `npm run e2e:bt` alone, `SOTTO_BT_CONDITIONS=db40,db45` to measure the level cliff.
