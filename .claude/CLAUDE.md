...
- `web/`: the voice page. No framework, no build step, no network requests except to the daemon. `string.js` is the canvas string under the whole panel (the peg, Claude's bead and milestone stars, the eclipse approval; guarded: a failure there must never break the page, and it draws nothing while the panel is idle); `panel.js` holds the DOM painters shared by app.js and the layout tests (like `header.js`). `lib.js` is pure and unit-tested (view models in `pageView()`); so is `wake.js` (local voice wake: VAD, pre-roll clip), and `echo.js` (echo measurement and the echo guard's DSP, run on the audio thread by `echo-worklet.js`; SPEC §7.7). Idle sleep/wake on the daemon side is `daemon/wake.js` + `daemon/transcribe.js` (SPEC §6.15, §7.6).
...
- **Keychain in tests:** unit tests use `test/helpers/fake-keychain.js` (the daemon harness default) or a fake `security` via `SOTTO_SECURITY_BIN`; anything that spawns a real daemon sets `SOTTO_KEYCHAIN_SERVICE` to a throwaway name so the user's real `sotto` item never leaks in. Never point a Keychain *write* at a fake `HOME`: `security` then fails with "authorization was canceled by the user".
  ...
  - It also runs `test/e2e/native-bt.mjs` (a Bluetooth headset mic on the native path: speech fixtures as AirPods deliver them in call mode (8 kHz band, gated -80 dBFS floor) at -31 and -50 dBFS and with keyboard typing between the words (typing must not be learned as speech), scored by gpt-live-1's word recall (the -50 condition needs the uplink AGC, SPEC §6.19), then a voice wake at the headset level whose clip must be transcribed and whose session must stay awake while the user talks; ~110 billed seconds); `npm run e2e:bt` alone, `SOTTO_BT_CONDITIONS=db40,db45` to measure the level cliff, `SOTTO_BT_REPLAY=<raw.wav>` to replay a `sotto debug capture` recording (SPEC §6.19 "Debug capture": the real mic, raw and as sent, for "it doesn't hear me" reports; delete the recordings afterwards).
  ...
  - It also runs `test/e2e/relay.mjs` (a markdown-heavy Stop result must be relayed in a few natural words: no "Claude Code's answer", no list, label or path read aloud, no tag line, under 45 words, key fact kept; ~25 billed seconds); `npm run e2e:relay` runs it alone.
