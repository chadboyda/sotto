...
- So on **headphones** the app captures the microphone itself (the one the user chose, else the system default, a Bluetooth headset's included: SPEC-DEVIATIONS "Bluetooth mic") with a plain input unit, no voice processing, and streams it into the page as an ordinary track. Headphones have no echo path, so nothing is lost and nothing is ducked (a Bluetooth mic in use puts the headset in call quality, as any call app does). On speakers the app uses Apple's canceller. `auto` picks the app with AirPods too.
